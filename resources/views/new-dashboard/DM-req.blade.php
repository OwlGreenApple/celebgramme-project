																...
																if (count($pendingInboxResponse->getInbox()->getThreads()) > 0 ) {
																	foreach ($pendingInboxResponse->getInbox()->getThreads() as $data_arr) {
																		...
																		if (strlen($text_message)>=42) {
																			$text_message = substr($text_message,0,60)." ...";
																		}
															?>
															<div class="row">
																	...
																	<div class="row">
																		<div class="col-md-4 col-sm-4 col-xs-4 br-6">
																			<div style="padding:5px;" class="bg-white br-6 text-center same-height">
																				<b class="text-primary" style="margin-top:7px;display:block">
																					{{date("l", $date_message)}}<br>{{date("Y-m-d", $date_message)}}
																				</b>
																			</div>
																		</div>
																		<div class="col-md-4 col-sm-4 col-xs-4 br-6">
																			<div style="padding:5px;cursor:pointer;" class="bgGreenLight text-center button-accept-request same-height" data-thread-id="{{$data_arr->getThreadId()}}" data-pic="{{$data_arr->getUsers()[0]->getProfilePicId()}}" href="#chat-all" data-toggle="tab"  data-username="{{$data_arr->getUsers()[0]->getUsername()}}" >
																				<i class="fa fa-check fa-2x" style="margin-top:7px;display:block"></i>
																				<b class="text-white">Accept</b>
																			</div>
																		</div>
																		<div class="col-md-4 col-sm-4 col-xs-4 br-6">
																			<div style="padding:5px;cursor:pointer;" class="bg-red text-center button-decline-request same-height" data-toggle="modal" data-target="#confirm-decline" data-thread-id="{{$data_arr->getThreadId()}}">
																				<i class="fa fa-times fa-2x" style="margin-top:7px;display:block"></i>
