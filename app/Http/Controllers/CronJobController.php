...
*/
  public function notif_member(){
	...
	public function create_user_from_affiliate(){
		...
		$datas = DB::connection('mysqlAffiliate')->select("select p.*,u.user_email,u.display_name from wp_af1posts p inner join wp_af1users u on u.id=p.post_author where post_title like 'CLB%' and post_content='' and p.post_type='wuoysales' and post_status='publish'");		
		// dd($datas);
		// echo $datas[0]->ID;
